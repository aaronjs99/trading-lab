...
def main() -> None:
    ...
    if not csv_files:
        parser.exit(
            status=1,
            message=(
                f"No Robinhood CSV files found under {input_dir}. "
                "Export your Robinhood history CSV and place it there.\n"
            ),
        )
    ...
    realized = calculate_fifo_realized_pnl(trades, allow_unmatched_sells=True) if not trades.empty else trades
