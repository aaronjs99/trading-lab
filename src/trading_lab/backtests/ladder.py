...
def run_tqqq_ladder_backtest(
    ...
    drawdown = target / rolling_high - 1.0
    ...
    metrics = equity_curve_metrics(
        results["equity"] if not results.empty else pd.Series(dtype=float)
    )
